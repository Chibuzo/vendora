...
import { coerceAuthPayload, phoneVerifySchema, verifyPhoneLogin } from '@/modules/auth/services/auth-service';
...
export async function POST(request: Request) {
  ...
  try {
    if (!env.NEXT_PUBLIC_ENABLE_MOCKS) {
      const upstream = await requestBackend<unknown>('/auth/login/phone/verify', {
        method: 'POST',
        body: parsed.data
      });
